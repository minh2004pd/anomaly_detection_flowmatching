...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        help="Type of learning rate scheduler: constant, linear decay, or cosine annealing.",
    )
    parser.add_argument(
        "--min_lr",
        type=float,
        default=1e-6,
        help="Minimum learning rate for cosine/linear decay schedulers.",
    )
    parser.add_argument(
        "--warmup_epochs",
        type=int,
        default=0,
        help="Number of warmup epochs (linear warmup from min_lr to lr).",
    )
    parser.add_argument(
        "--skewed_timesteps",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help="Number of samples in the simple shape dataset.",
    )
    parser.add_argument(
        "--image_size",
        default=64,
        type=int,
        help="Size of the simple shape images.",
    )
    ...
    parser.set_defaults(compute_fid=False)
    parser.add_argument(
        "--save_fid_samples",
        action="store_true",
    ...
    parser.add_argument(
        "--healthy_only",
        action="store_true",
        help="Train only on healthy data. Tumor regions in unhealthy slices are inpainted.",
    )
    parser.add_argument(
        "--use_preprocessed",
        action="store_true",
        help="Use preprocessed .npy dataset instead of raw .nii.gz",
    )
